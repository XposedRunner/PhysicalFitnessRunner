...
public final class ColorUtils {
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        switch (((int) f) / 60) {
            ...
            case 2:
                ...
                round2 = Math.round((abs + f3) * 255.0f);
                round3 = Math.round(255.0f * (abs2 + f3));
                break;
            case 3:
                round = Math.round(255.0f * f3);
                round2 = Math.round((abs2 + f3) * 255.0f);
                round3 = Math.round(255.0f * (abs + f3));
                break;
            case 4:
                round = Math.round((abs2 + f3) * 255.0f);
    ...
    public static void RGBToXYZ(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull double[] dArr) {
        ...
        dArr2[2] = XYZ_WHITE_REFERENCE_Y * (((pow * 0.0193d) + (pow2 * 0.1192d)) + (d * 0.9505d));
    }
