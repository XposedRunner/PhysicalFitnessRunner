package com.amap.api.mapcore.util;

import com.amap.api.mapcore.util.o0OO0o00.O000000o;

/* compiled from: ConstConfig */
public class oO0Ooo00 {
    static String O000000o = "CUlNBL0VDQi9QS0NTMVBhZGRpbmc";
    public static String O00000Oo = "9aj&#k81";
    static byte[] O00000o = null;
    public static String O00000o0 = "IaHR0cDovL2xvZ3MuYW1hcC5jb20vd3MvbG9nL3VwbG9hZD9wcm9kdWN0PSVzJnR5cGU9JXMmcGxhdGZvcm09JXMmY2hhbm5lbD0lcyZzaWduPSVz";
    private static String O00000oO = "";

    public static o0OO0o00 O000000o() {
        try {
            return new O000000o("aiu", com.ecloud.pulltozoomview.O000000o.O00000oo, "aiu").O000000o(new String[0]).O000000o();
        } catch (o0O0oo0o e) {
            e.printStackTrace();
            return null;
        }
    }

    public static byte[] O000000o(int i) {
        if (O00000o == null) {
            O00000o = o0OO000o.O00000Oo(O00000oO);
        }
        byte[] bArr = new byte[4];
        System.arraycopy(O00000o, i * 4, bArr, 0, 4);
        i = ((bArr[0] & 255) * 256) + (bArr[1] & 255);
        int i2 = ((bArr[2] & 255) * 256) + (bArr[3] & 255);
        bArr = new byte[i2];
        System.arraycopy(O00000o, i, bArr, 0, i2);
        return bArr;
    }
}
