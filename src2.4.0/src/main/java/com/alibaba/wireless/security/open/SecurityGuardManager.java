...
public class SecurityGuardManager {
        ...
        try {
            synchronized (g) {
                if (str == null && str2 == null) {
                    ...
                    throw new SecException(SecExceptionCode.SEC_ERROR_INIT_INVALID_PARAM);
                }
            }
            return true;
        } catch (JSONException e) {
            throw new SecException(e.getMessage(), (int) SecExceptionCode.SEC_ERROR_SET_GLOBAL_USER_DATA);
        } catch (Throwable th) {
        }
    }
    ...
    public IComponent a(int i) {
        try {
            return (IComponent) getInterface((Class) this.f.get(Integer.valueOf(i)));
